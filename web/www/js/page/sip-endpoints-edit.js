...
(function(){
	...
	$("#lang_stun_switch_help").html(language('STUN Switch help','You need to turn on the stun switch on the SIP advanced settings page to take effect'));
...
function show_sip(sip_info, all_sip_info){
	...
	if(sip_info['_stunflag'] == 1){
		var stun_switch = 'checked';
	}else{
		var stun_switch = '';
	}
	...
	document.getElementById('stun_switch').checked = stun_switch;
...
function save_endpoint(all_sip_info){
	...
	if(document.getElementById('stun_switch').checked){
		var stun_switch = 1;
	}else{
		var stun_switch = 0;
	}
	sipcontext._stunflag = stun_switch;
