...
SSL_PREFIX=$BUILDFS_DIR
SDK_DIR=
SDK_DIR_INC=${SDK_DIR}/usr/include
SDK_DIR_LIB=${SDK_DIR}/usr/lib
...
compile_cgicc()
{
	...
	./configure --prefix=${BUILDFS_DIR}/usr 
	if [ x"$?" != x"0" ];then
...
compile_fcgi()
{
	...
	./configure --prefix=${BUILDFS_DIR}/usr CFLAGS="-g -O2 -Wall -fPIC" CXXFLAGS="-g -O2 -Wall -fPIC"
	if [ x"$?" != x"0" ];then
		error "${app} configure failed"
	fi
...
compile_pcre()
{
	...
	./configure --prefix=${BUILDFS_DIR}/usr CPPFLAGS=-I${BUILDFS_DIR}/usr/include LDFLAGS=-L${BUILDFS_DIR}/usr/lib
	if [ x"$?" != x"0" ];then
		error "${app} configure failed"
	fi
...
#	./configure --prefix=${LIGHTTPD_OUTPUT}/usr --without-bzip2 --with-openssl --with-openssl-libs=/usr/lib64 CFLAGS="-I${BUILDFS_DIR}/usr/include -I${SDK_INCLUDE_DIR}" LDFLAGS="-L${BUILDFS_DIR}/usr/lib -L${SDK_LIB_DIR}"
#	./configure --without-bzip2 --with-openssl=${SSL_PREFIX}/usr CFLAGS="-I${BUILDFS_DIR}/usr/include -I${SDK_DIR_INC}" LDFLAGS="-L${BUILDFS_DIR}/usr/lib -L${SDK_DIR_LIB}"
	./configure --without-bzip2 --with-openssl=${SSL_PREFIX}/usr
	if [ x"$?" != x"0" ];then
		error "${app} configure failed"
	fi
...
#	./configure --host=${GCC_PREFIX} --prefix=/ --enable-mbstring  --with-sqlite --with-sqlite3 --disable-all --with-openssl=$PREFIX_PATH/usr --enable-pcntl --enable-json --enable-session --enable-sockets --enable-gd-native-ttf --enable-gd-jis-conv --enable-posix --enable-sysvmsg --enable-sysvshm --with-iconv=$PREFIX_PATH/usr --with-curl=$PREFIX_PATH
	./configure --prefix=/ --enable-mbstring  --with-sqlite --with-sqlite3 --disable-all --with-openssl=${SSL_PREFIX}/usr --enable-pcntl --enable-json --enable-session --enable-sockets --enable-gd-native-ttf --enable-gd-jis-conv --enable-posix --enable-sysvmsg --enable-sysvshm --with-iconv=${BUILDFS_DIR}/usr --with-curl=${BUILDFS_DIR}/usr LIBS="-ldl" 
	if [ x"$?" != x"0" ];then
		error "${app} configure failed"
	fi
