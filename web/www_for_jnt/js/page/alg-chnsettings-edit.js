...
function check(all_analog_data,sip_data,routing_data,chnl_type)
{
	...
	var speed_dial_flag = false;
	$(".speeddial_number").each(function(){
		if($(this).val() == ''){
			$(this).siblings('.cspeeddial_number').html(con_str('Can not be null'));
			$(this).siblings('.cspeeddial_number').innerHTML = con_str('Can not be null');
			$(this).focus();
			speed_dial_flag = true;
		}
	});
...
function add_more_speed_dial_options(all_analog_data){
	var speeddial_port_select = '<select name="speeddial_port" class="speeddial_port" style="margin-left:5px;">';
		for(var item in all_analog_data){
			if(all_analog_data[item]['_signalling'] == 1){//FXS
				speeddial_port_select += '<option value="'+all_analog_data[item]['_channel']+'" >S'+all_analog_data[item]['_channel']+'</option>';
			}
		}
		speeddial_port_select += '</select>';
	
	var str = '<tr>'+
				'<td>'+
					'<input type="text" name="speeddial_number" class="speeddial_number" style="margin-left:5px;" value="" style="width:120px;" />'+
				'</td>'+
				'<td>'+
					speeddial_port_select+
					'<span class="cspeeddial_number"></span>'+
				'</td>'+
				'<td>'+
					'<button class="delete_sppeddial" type="button" style="width:32px;height:32px;">'+
...
function show_speeddial(data_temp){
	var speeddial_data = data_temp['_speeddial']['_item'];
	var all_analog_data = data_temp['_section']['_item'];
	
	var speeddial_str = '<table width="100%" class="tshow" id="tab_lnl">'+
			'<tr>'+
				'<th style="text-align:left;">'+ language('Number') +'</th>'+
				'<th style="text-align:left;">'+ language('Port') +'</th>'+
				'<th style="text-align:left;width:10%;">'+ language('Actions') +'</th>'+
			'</tr>';
	for(var i=0;i<speeddial_data.length;i++){
		
		var speeddial_port_select = '<select name="speeddial_port" class="speeddial_port" style="margin-left:5px;">';
		for(var item in all_analog_data){
			if(all_analog_data[item]['_signalling'] == 1){//FXS
				var selected = '';
				if(all_analog_data[item]['_channel'] == speeddial_data[i]._fxsport){
					selected = 'selected';
				}
				speeddial_port_select += '<option value="'+all_analog_data[item]['_channel']+'" '+selected+' >S'+all_analog_data[item]['_channel']+'</option>';
			}
		}
		speeddial_port_select += '</select>';
		...
		speeddial_str +=
			'<tr>'+
				'<td>'+
					'<input type="text" name="speeddial_number" class="speeddial_number" style="margin-left:5px;" value="'+speeddial_data[i]._dialnumber+'" style="width:120px;" />'+
				'</td>'+
				'<td>'+
					speeddial_port_select+
					'<span class="cspeeddial_number"></span>'+
	...
	return speeddial_str;
}

$(document).on('click','.delete_sppeddial',function(){
...
function edit_chnl(data_temp){
	...
	chnl.chnnl_type = url['chnl_type'] == 's' ? 'S Ports' : url['chnl_type'];
	...
	if(analog_data['_telephonetype'] != ''){
		chnl.telephone_type = analog_data['_telephonetype'];
	}
	...
	if(chnl.chnnl_type=='S Ports'){
		...
		$("#tab_general table").append(
			'<tr>'+
				'<th>'+
					'<div class="helptooltips">'+language('Telephone type')+':'+
						'<span class="showhelp">'+language('Telephone type help','Telephone type')+'</span>'+
					'</div>'+
				'</th>'+
				'<td>'+
					'<select name="telephone_type" id="telephone_type">'+
						'<option value="0">'+ language('2 Line') +'</option>'+
						'<option value="1">'+ language('4 Line') +'</option>'+
					'</select>'+
				'</td>'+
			'</tr>'
		);
		
		document.getElementById('telephone_type').value = chnl.telephone_type;
	...
	/* Speed Dial begin */
	if(chnl.chnnl_type=='S Ports'){
		var speed_dial_str = '<div id="newline"></div>'+
			'<div id="tab">'+
				'<li class="tb_unfold" onclick="lud(this,\'tab_calleriddetect\')" id="tab_calleriddetect_li">&nbsp;</li>'+
				'<li class="tbg_fold" id="lang_speed_dial" onclick="lud(this,\'tab_calleriddetect\')"></li>'+
				'<li class="tb2_fold" onclick="lud(this,\'tab_calleriddetect\')">&nbsp;</li>'+
				'<li class="tb_end">&nbsp;</li>'+
			'</div>'+
			...
			'<div id="tab_calleriddetect">'+
				'<table width="98%" class="tedit" align="right">'+
					'<tr>'+
						'<th>'+
							'<div class="helptooltips">'+
								'<span id="lang_timeout">'+language('Timeout')+'</span>:'+
								'<span class="showhelp" id="lang_timeout_help">'+language('Timeout help','Timeout')+'</span>'+
							'</div>'+
						'</th>'+
						'<td>'+
							'<input type="text" name="timeout" id="timeout" value="'+analog_data['_speeddailtimeout']+'"/>'+
							'<span id="ctimeout"></span>'+
						'</td>'+
					'</tr>'+
					...
					'<tr>'+
						'<th>'+
							'<div class="helptooltips">'+
								'<span>'+language('Speed Dial')+'</span>:'+
								'<span class="showhelp">'+language('Speed Dial help','Speed Dial')+'</span>'+
							'</div>'+
						'</th>'+
						'<td>'+
							show_speeddial(data_temp)+
						'</td>'+
					'</tr>'+
					...
					'<tr>'+
						...
						'</th>'+
						'<td>'+
							'<button type="button" id="add_more_speed" style="width:32px;height:32px;"><img src="../images/add-new.png" /></button>'+
						'</td>'+
					'</tr>'+
				'</table>'+
			'</div>';
			
			$("#tab_limit").after(speed_dial_str);
			
			$("#add_more_speed").click(function(){
				add_more_speed_dial_options(all_analog_data)
			})
			
			$("#lang_speed_dial").html(language('Speed Dial'));
	}
	/* Speed Dial end */
	...
	for(t in all_analog_data){
		...
		if(chnl.chnnl_type=='S Ports'){
			if(all_analog_data[t]['_signalling'] == 1){//1:fxo_ks
				if(k%4!=1 || k==1){
					if(channel==each_chn){
						_port_str += '<td><input type="checkbox" checked disabled>S'+each_chn+'</td>';
					}else{
						_port_str += '<td><input type="checkbox" class="port" name="spans['+each_chn+']" value="'+each_chn+'">S'+each_chn+'</td>';
					}
				}
				if(k%4==1 && k!=1){
					if(channel==each_chn){
						_port_str += '</tr><tr><td><input type="checkbox" checked disabled>S'+each_chn+'</td>';
					}else{
						_port_str += '</tr><tr><td><input type="checkbox" class="port" name="spans['+each_chn+']" value="'+each_chn+'">S'+each_chn+'</td>';
					}
				}
				k++;
			}
	...
	for(var item in all_analog_data){
		...
		if(all_analog_data[item]['_signalling'] == 1){
			sidebar_html += '<li><a style="'+item_color+'" href="alg-chnsettings-edit.html?chnl_num='+all_analog_data[item]['_channel']+'&chnl_type=s" >S'+all_analog_data[item]['_channel']+'</a></li>';
		}else{
...
function save_channel(analog_data, flex_routing_sw){
	...
	if(chnl_type == 's'){
		...
		var telephone_type = document.getElementById('telephone_type').value;
		anacontextfxoks._telephonetype = telephone_type;
		...
		var speeddialtimeout = document.getElementById('timeout').value;
		anacontextfxoks._speeddailtimeout = speeddialtimeout;
	...
	var speed_arr = [];
	$(".speeddial_port").each(function(){
		var speed_port = $(this).val();
		speed_arr.push({'number':'','port':speed_port});
	});
	
	var n = 0;
	$(".speeddial_number").each(function(){
		var speed_number = $(this).val();
		speed_arr[n].number = speed_number;
		n++;
	});

	for(var i=0;i<speed_arr.length;i++){
		var FxsSpeeddial = new AST_FxsSpeeddial();
		FxsSpeeddial._fxsport = speed_arr[i].port;
		FxsSpeeddial._dialnumber = speed_arr[i].number;
		Speeddial._item.push(FxsSpeeddial);
	}
	/* speed dial end */
