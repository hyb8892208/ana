function check(data_temp){
	if(document.getElementById('flex_routing_sw').checked == true && data_temp['_combuf']['_FlexRoutingSw'] == 0){
		if(!confirm(language('Flex Routing Switch Open help', 'This operation clears all S Port bound SIP extension information.Need to continue?'))){
			return false;
		}
	}else if(document.getElementById('flex_routing_sw').checked == false && data_temp['_combuf']['_FlexRoutingSw'] == 1){
		if(!confirm(language('Flex Routing Switch Close help', 'This operation clears all S Port related routing information.Need to continue?'))){
			return false;
		}
	}
...
(function(){
	...
	$("#lang_internalcallsw").html(language('Enable Internal S Port Call'));
	$("#lang_internalcallsw_help").html(language('Enable Internal S Port Call help', 'Enable Internal S Port Call'));
