function check(global_data){
	...
	if(new_sigfxs != old_sigfxs){
		var ret = confirm(language('S Ports Signaling change','Modifying the S Ports signaling parameter will clear all channel bound sip, which must be restarted to take effect.'));
		if(!ret){
			return false;
		}
	}
...
(function(){
	...
	$("#lang_fxs_signaling").html(language('S Ports Signaling'));
	$("#lang_fxs_signaling_help").html(language('S Ports Signaling help','S Ports Signaling'));
	/* General end */
