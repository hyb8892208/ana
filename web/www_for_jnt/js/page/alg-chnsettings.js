...
function show_chnls(analog_data, channel_data, sip_data, routing_data, calllimit_data, flex_routing_sw){
	...
	if(n==0){
		...
		for(var item in analog_data){
			...
			if(analog[item]['sigtype'] == 'S Ports'){
				if(flex_routing_sw == 0){
			...
			html_str +=	'<td style="text-align:center;"><a href="alg-chnsettings-edit.html?chnl_num='+analog[item]['channel']+'&chnl_type='+s_type+'" onclick="return check_channel('+analog_data[item]['_signalling']+');javascript:window.location.href=this.href"><button style="width:32px;height:32px;"><img src="../images/edit.gif"></button></a></td>';
