...
var title = {
	...
	"routing" : {
		...
		"Ring Groups": "routing-ring-groups.html",
		"Ring Groups Edit": "routing-ring-groups-edit.html",
	},
	"network" : {
	...
	"advanced" : {
		...
		"SNMP" : "adv-snmp.html",
		"Auto Provision" : "adv-autoprovision.html",
	},
	"logs" : {
...
function Navigation(data_temp){
	...
	for(var item in title){
		...
		for(var name in title[item]){
			...
			if(type==0 && (name=='S Ports' || name=='S Ports Batch Binding SIP' || name=='Dial Matching Table' || name=='Pickup Settings' || name=='Special Function Keys')){continue;}
			...
			if(url=='routing-ring-groups-edit.html' && title[item][name]=='routing-ring-groups-edit.html'){continue;}
			if(name=='Ring Groups Edit'){continue;}
...
function header(data_temp){
	...
	if(flex_routing_sw == 1){
		delete title['sip']['S Ports Batch Binding SIP'];
	}
